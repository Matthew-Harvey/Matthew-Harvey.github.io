...
class both: # contains functions / classes used by various sections
    ...
    class changelogin:
        def __init__(self, IDENT_insert):
            ...
            self.__tab4 = ttk.Frame(tabcontrol)
            tabcontrol.add(self.__tab4, text="Step 2")
            tabcontrol.select(self.__tab4) # selecting the new tkinter tab
            self.__ID_insert = IDENT_insert
            HoverButton(self.__tab4, height=2, width=30, font="Verdana 20", activebackground='Light Yellow', fg="Black", bg="Yellow", text=" edit your data ", command = lambda: both.changingdata(self.__ID_insert, self.__tab4)).grid(row=2) # redirect for the function, for the user to change any information
            tk.Label(self.__tab4, text=" ").grid(row=1)
            tk.Label(self.__tab4, text=" ").grid(row=3) # formatting with blank labels
            tk.Label(self.__tab4, text=" ").grid(row=10)
            HoverButton(self.__tab4, height=2, width=30, font="Verdana 20", activebackground='Light Grey', fg="white", bg="Dark Grey", text=" Previous Recommendations ", command = lambda: [both.viewing_all(self.__ID_insert), both.closetab(self.__tab4)]).grid(row=6) # redirecting to a function that handles all of the recommendations processed 
            HoverButton(self.__tab4, text="Back to Main Menu", activebackground='blue', fg="white", bg="black", command= lambda: [both.closetab(self.__tab4), both.home(IDENT_insert)]).grid(row=11) # back to logged in home button
...
class job: # making a job class to catagorise all of the functions
    ...
    class jobcentre:
        ...
        def distance(self, longit1, lat1, longit2, lat2):
            ...
            step2 = step2 * step2 # alternative to writing (step2)**2
            step3 = step1 + step2
            step3 = (step3)**0.5 # square root by doing to the power of a half
            return step3
    ...
    def display(self, job_name, countcheck, ID_insert):
        ...
        handle.commit()
        if len(count) > 1: # retreiving the number of recommendations a job has, to decide on whether to show user traits and also part of the inital UI
            count = count[0]
        count = str(count)
        count = re.sub("[^0-9]", "", count) # casting into an integer from an inital tuple that is obtained from the database
        if count == "":
            count = 1
        else:
            try:
                count=int(count)
                if countcheck == 1: # only adding a recommendation if the function was sent from specific ways, such as being recommended through the "find jobs" buttons or just most popular etc.
                    count = count+1
            except:
                count = "N/A"
        rec_count = count
        if count > 1:
            HoverButton(result_tab, activebackground='Light Blue', fg="Black", bg="White", text="Similar User Traits", command= lambda: job.sim_users(job_name, ID_insert)).grid(row=22, column=1) # only showing if other users have previously been recommended this job
        ...
        cursor.execute("UPDATE JOBS SET COUNT = ? WHERE JOBNAME LIKE ? AND JOBID > ?", (count, "%"+job_name+"%", "0")) # updating the counts for the job, the only way it changes is through it being recommended to the user
        handle.commit()
        employees = str(employees)
        employees = re.sub("[^0-9]", "", employees) # formatting the amount of people employed into a string and being placed into a sentence to be displayed in a tkinter label
        ...
        tk.Label(result_tab, text="Field:", font= "Ariel 12", fg="Dark Blue").grid(row=11) # structural labels
        ...
        cursor.execute("SELECT DESCRIPTION FROM JOBS WHERE JOBNAME LIKE ? AND JOBID > ?", ("%"+job_name+"%", "0")) # retreiving the description of the job from the database
        descript = cursor.fetchall()
        handle.commit()
        if len(descript) > 1:
            descript = descript[0] # stripping out unwanted sections of a tuple
        descript = str(descript)
        descript = descript[3:len(descript)-4]
        descript = descript.strip()
        descript = descript.replace("Expand", "") # expand was included in the html file and had to be removed after as the database is already set up with this small issue
        descript = descript.strip()
        try:
            des_test = "'"+descript[0]+"'"
            hold = "'\\'"
            if des_test == hold:
                descript = descript[12:] # simple formatting of the string to remove certain characters or regions
        except:
            cont = True # placeholder
        ...
        if len(salary) > 1:
            salary = salary[0]
        salary = str(salary)
        salary = re.sub("[^0-9]", "", salary) # recieve the salary from the database and ensure it is as a string to be added to $, to make a valid amount
        number_hold = salary
        salary = "$"+salary
        ...
        HoverButton(result_tab, activebackground='light blue', fg="Black", bg="White", text="DESCRIPTION", command = lambda: [job.displaydescript(descript, job_name, ID_insert), both.closetab(result_tab)]).grid(row=7, column=1) # buttons to see additional static data
        ...
        string = str(skill)
        for i in string:
            if i not in punctuation: # get data about the skills of the specific job from the database and proceed to manipulate the strings and strip back into parts, containing one skill each
                skills += i
        skills = skills.split(".,")
        ...
        if len(field) > 1:
            field = field[0]
        field = str(field)
        field = field[2:len(field)-5] # another retreival of data from the database before being manipulated from a tuple into a string while removing various regions of the new string
        try:
        ...
        HoverButton(result_tab, activebackground='light blue', fg="Black", bg="White", text="Field Info", command = lambda: job.field_info(result_tab, field, job_name, ID_insert)).grid(row=13, column=1) # button to provide more information about the specific job role
        ...
        HoverButton(result_tab, activebackground='light blue', fg="Black", bg="White", text="Qual Split", command = lambda: job.qual_split(job_name)).grid(row=17, column=1) # button to run function that will evaluate ratios of qualification levels
        ...
        HoverButton(result_tab, activebackground='light blue', fg="Black", bg="White", text="Rate this job", command = lambda: job.ratingadd(job_name, result_tab, ID_insert)).grid(row=21, column=1) # function to make a new tab to rate the job as there is no more available space on the pre-existing tkinter tab
        ...
        HoverButton(result_tab, activebackground='Blue', fg="Blue", bg="White", text="CLOSE TAB", command = lambda: both.closetab(result_tab)).grid(row=27) # standard navigation buttons
    ...
    def sim_users(self, job_name, ID_insert):
        ...
        jobid = cursor.fetchall()
        if len(jobid) > 1:
            jobid = jobid[0] # get the job id to make the following requests easier, casting into a string from tuple
        jobid = str(jobid)
        ...
        for y in range(0, len(all_skills)):
            ...
            for j in range(0, len(all_skills)): # perform the same operation again to see which skill name has the most repeats - that matches the amount recored in previous loop
                if all_skills[y] == all_skills[j]:
                    count = count+1
            if count == topcount:
                topskill = all_skills[y] # this attains the top skill to be shown in a label
        all_non_duplicated_skills = []
        nums = []
        for l in range(0, len(all_skills)):
            if all_skills[l] not in all_non_duplicated_skills:
                all_non_duplicated_skills.append(all_skills[l]) # making a list of skills without duplicates and each skills count attached in an array to make a pi chart in the "skillsplit" function
        for y in range(0, len(all_non_duplicated_skills)):
            ...
            for j in range(0, len(all_skills)):
                if all_non_duplicated_skills[y] == all_skills[j]:
                    count = count+1
            nums.append(count)
        HoverButton(similarusers, activebackground='blue', fg="white", bg="black", text="Skill Split", command = lambda: job.skillsplit(all_non_duplicated_skills, nums, ID_insert)).grid(row=3) # using generated array to make a pi chart in another function
        tk.Label(similarusers, text =job_name).grid(row=1)
        tk.Label(similarusers, text ="Top Skill: ").grid(row=2) # labels to provide the crucial information to the user and provide some structural layouts
    ...
    def skillsplit(self, all_non_duplicated_skills, nums, ID_insert):
        try:
            import matplotlib.pyplot as plt # importing these modules as they are used to make the pi charts clean and have a easy to create system
            import numpy as np
        except:
            cont = True
        labels = all_non_duplicated_skills # list of all skills that should be included
        sizes = nums # list of all numbers that correspond to relevant sizing ratios
        explode = [0.5]*len(sizes) # amount each slice is apart in the diagram
        fig1, ax1 = plt.subplots()
        ax1.pie(nums, explode=explode, labels=all_non_duplicated_skills, autopct='%1.1f%%', shadow=True, startangle=90) # creating the pi chart with some static variables that affect how its shown to the user
        ax1.axis('equal')
        plt.suptitle("Skill Split", fontsize = 22) # pi chart title
        plt.show() # finalise pi chart and hence create it in a new window
    ...
    def displaydescript(self, descript, job_name, ID_insert):
        ...
        count2 = 0
        count3 = []
        for char in descript: # using the description given as a parameter to display in a tab to the user
            count2 +=1
            if char == " ":
        ...
        if len(count3) >= 10 and len(count3) <= 19:
            ...
            tk.Label(result_tab, text=descript[count3[9]+1:]).grid(row=4, columnspan=7) # these are used to split the description after every word so words 10-19 will be on specific lines to stop it overrunning the tab sizing of 500 by 500
        elif len(count3) > 19:
            tk.Label(result_tab, text=descript[:count3[9]]).grid(row=3, columnspan=7)
        ...
        tk.Label(result_tab, font="Ariel 15", fg="Light Blue", text="Description: ").grid(row=1, columnspan=7) # creating labels and buttons to provide structure and names for all associated sections
        tk.Label(result_tab, text=" ").grid(row=2)
        tk.Label(result_tab, text=" ").grid(row=19)
        HoverButton(result_tab, activebackground='Light Blue', fg="white", bg="black", text="Back", command = lambda: [both.closetab(result_tab), job.display(job_name, 0, ID_insert)]).grid(row=20, columnspan=7) # just a close tab as it opens without closing the original job specific page
   
    def RECtemplate(self, job_name, result_tab, field, descript, skills, salary, ID_insert):
        try:
            from fpdf import FPDF # using this import to create and format the pdf file as it cannot be done efficently without the use of a module
            file_handle = job_name+".pdf"
            pdf = FPDF()
            pdf.add_page()
            pdf.set_font("Arial", size=8)
            pdf.cell(200, 10, txt=job_name, ln=1, align="C") # creating cells that contain each piece of data that is needed to be shown to the user in a pdf format
            ...
            linesplit = 0
            condition = ['none,']
            if skills != condition: # if the job requires specific skills, these are added as cells in a smaller font due to the length of some of these skills descriptions
                pdf.cell(200, 10, txt="Skills:", ln=7, align="L")
                pdf.set_font("Arial", size=6)
                for c in range(0, len(skills)):
                    pdf.cell(200, 10, txt=skills[c], ln=linesplit+8, align="L") # making a new cell for each skill
            ...
            tk.Message(result_tab, text="File error, file could not be produced.").grid(row=8, column=1) # if there is any issues in this process, show this on the main job specific page
        
    def similar_jobs(self, field, job_name, result_tab, ID_insert):
        result_tab.destroy()
        handle = sql.connect("RecommendDATA.db")
        cursor = handle.cursor()
        cursor.execute("SELECT JOBNAME FROM JOBS WHERE FIELD LIKE ? AND JOBID > ?", ("%"+field+"%", "0")) # getting a list of all the jobs with an identical field
        ...
        for b in range(0, len(joblist)):
            ...
            new = ""
            for i in string:
                if i not in punctuation: # removing all the issues with tuples into an array of job names
                    new = new+i
            new = new[:len(new)-2]
            fin.append(new)
        ...
        if len(quals)>1:
            quals = quals[1]
        nameofqual = job.findcommonqual(quals) # finding the qualification level of the current selected job
        final = []
        for i in range(0, len(fin)):
            cursor.execute("SELECT QUALLEVEL FROM JOBS WHERE JOBNAME LIKE ? AND JOBID > ?", ("%"+fin[i]+"%", "0"))
            jobqual = cursor.fetchall()
            handle.commit()
            jobqualname = job.findcommonqual(jobqual) # finding the qualification level of the potential job, if the two match then it will be added to an array to be potentially selected by the user
            if jobqualname == nameofqual:
                final.append(fin[i])
        if final == []:
            while len(fin) > 5:
                fin.remove(fin[random.randint(0, len(fin)-1)]) # maximum of 5 other jobs can be recommended to the user as a similar role
            final = fin
        ...
        tk.Label(similar_tab, text ="Similar roles: ").grid(row=2, column=4) # creating a tkinter tab to show the user some similar roles in an option menu and then confimation into another function "getvar3"
        tk.OptionMenu(similar_tab, var3, *fin).grid(row=2, column=5)
        HoverButton(similar_tab, activebackground='blue', fg="white", bg="black", text="Continue", command = lambda: [both.getvar(var3, ID_insert), both.closetab(similar_tab)]).grid(row=3) # interpreting input and then redirecting to the job specific page
    ...
    def ratingadd(self, job_name, result_tab, ID_insert):
        ...
        result_tab = ttk.Frame(tabcontrol)
        title = "Rate " + job_name
        tabcontrol.add(result_tab, text=title) # creating a new tkinter tab for the user to input a rating and a comment to be inserted into the database in the function ("ratingadd2")
        tabcontrol.select(result_tab)
        one_to_five = [1, 2, 3, 4, 5]
        handle = sql.connect("RecommendDATA.db")
        cursor = handle.cursor()
        ...
        cursor.execute("SELECT RATING FROM RATINGS WHERE JOBID = ? AND JOBID > ?", (jobid, 0)) # getting the job id, to then retreive the rating in the ratings table
        ratings = cursor.fetchall()
        handle.commit()
        handle.close()
        total = 0
        ratings = re.sub('[^0-9]', '', str(ratings)) # a string of individual ratings complied on after another eg. 2879534 where 2, 8, 7, 9, 5, 3, 4 are seperate ratings
        try:
            for rate in ratings:
                total = total + int(rate)
            total = total / len(ratings) # finding the average rating for the entire job
        except:
            total = 3
        average = total
        average = str(average)
        av_rate = "Average rating: " + average
        tk.Label(result_tab, text =av_rate).grid(row=1, column=5) # displaying the current average rating to the user
        stars = IntVar()
        stars.set(3)
        ...
        comment = tk.Entry(result_tab) # allowing the user to type any issues etc. in the comment box
        comment.grid(row=3, column=5)
        HoverButton(result_tab, activebackground='blue', fg="white", bg="black", text="Back", command = lambda: [job.display(job_name, 0, ID_insert), both.closetab(result_tab)]).grid(row=5, column=4) # back without recording what is currently entered
        HoverButton(result_tab, activebackground='blue', fg="white", bg="black", text="Enter", command = lambda: job.ratingadd2(job_name, comment, stars, result_tab, ID_insert)).grid(row=5, column=5) # record all the data and then return to job specifc page

    def ratingadd2(self, job_name, comment, stars, result_tab, ID_insert):
        stars_ = stars.get()
        comment_ = comment.get() # interpreting what was entered into the tkinter option menu and entry box
        handle = sql.connect("RecommendDATA.db")
        cursor = handle.cursor()
        cursor.execute("SELECT JOBID FROM JOBS WHERE JOBNAME LIKE ? AND IDLE = ?", ("%"+job_name+"%", 0))
        jobID = cursor.fetchall()
        handle.commit()
        if len(jobID) > 1:
            jobID = jobID[1] # attaining the job id from the job name and then making it into a string from a tuple
        jobID = str(jobID)
        jobID = jobID[2:len(jobID)-3]
        cursor.execute("INSERT INTO RATINGS VALUES(?,?,?)", (jobID, stars_, comment_)) # inserting the job id, comment and rating into the table which alters the average rating
        handle.commit()
        handle.close()
        result_tab.destroy()
        job.display(job_name, 0, ID_insert) # returning to job specific homepage
    ...
    def qual_split(self, job_name):
        ...
        handle = sql.connect("RecommendDATA.db")
        cursor = handle.cursor()
        cursor.execute("SELECT QUALLEVEL FROM JOBS WHERE JOBNAME LIKE ? AND JOBID > ?", ("%"+job_name+"%", "0")) # selecting the data stored in the database under qual-level for that job described by the jobname variable
        qualforjob = cursor.fetchall()
        handle.commit()
        handle.close()
        qualforjob = str(qualforjob)
        ...
        for i in range(0, length):
            ...
            nums.append(quals[repeat+1]) # seperating the names of the qual-levels and the numbers that correspond to percentage ratios
            repeat = repeat+2
        labels = names
        sizes = nums
        explode = [0.5]*len(sizes)
        fig1, ax1 = plt.subplots()
        ax1.pie(nums, explode=explode, labels=names, autopct='%1.1f%%', shadow=True, startangle=90) # using module to create the pi chart in a new window, under specific variables that make it clear for the user
        ax1.axis('equal')
        plt.suptitle("Quallevel Split", fontsize = 22) # naming figure 1 as qual-level split
        plt.show() # finalising the data above and showing the user the pi chart
    ...
    def field_info(self, result_tab, field, job_name, ID_insert):
        ...
        handle = sql.connect("RecommendDATA.db")
        cursor = handle.cursor()
        cursor.execute("SELECT COUNT FROM JOBS WHERE IDLE = ? AND FIELD LIKE ?", (0, "%"+field+"%")) # attaining the amount of recommendations the whole field has
        ...
        for i in range(0, len(counts)):
            try:
                ...
                counts[i] = counts[i][1:len(counts[i])-2] # calculating the total amount of recommendations across the field
        ...
        tk.Label(fieldinfo_tab, text=total_count).grid(row=2) # creating a label to show the data to the user
        ...
        for name in names:
            name = str(name)
            name = name[2:len(name)-5] # get the job names for a field and then append them all into an array
            all_names.append(name)
        vars_ = StringVar()
        vars_.set(all_names[random.randint(0, len(all_names)-1)])
        tk.Label(fieldinfo_tab, text="All jobs in the field:").grid(row=3) # creating an option menu that the user can choose from, consisting of many job names
    ...
    def currency(self, number_hold, result_tab, job_name, salary, ID_insert):
        result_tab.destroy()
        urlof_live = "https://transferwise.com/gb/currency-converter/usd-to-~-rate?amount=#" # using the url structure to put inputs into url and search for output on the webpage
        ...
        list_of_currency = ["GBP", "INR", "ISK", "CAD", "EUR", "SDG", "AUD", "MXN", "SEK", "NOK", "JPY", "BRL"] # all the options that can be used to translate from USD to any selcection
        vars_ = StringVar()
        vars_.set(list_of_currency[random.randint(0, len(list_of_currency)-1)]) # setting a random currency as default as there are no sorting of currencies
    ...
    def use_currency(self, urlof_live, currency_tab, job_name, number_hold, vars_, ID_insert, salary):
        ...
        vars_ = vars_.get() # interpret input in the previous function "currency"
        vars2 = vars_
        try:
            vars_ = vars_.lower()
            urlof_live = urlof_live.replace("~", vars_) # manipulate the url string into a new url by replacing specific characters
            try:
                from bs4 import BeautifulSoup # modules that allow access to html parser of any url
                import requests
            except:
            ...
            soup = BeautifulSoup(response.content, "html.parser") # gathering the html file
            soup.prettify() # cleaning the html file
            sal = soup.find("div", {"class": "col-xs-12 col-md-6 text-xs-center"}).text # searching for a specific element in the html parser
            new_sal = ""
            for y in range(0, len(sal)):
                try:
                    ...
                    if sal[y+1] == ".":
                        new_sal = new_sal+sal[y+1] # manipulating the string given in the element by only compiling integers and if the integer is before a . it will add the dot aswell, to get the decimal place in the right place
                except:
                    cont = True
            sal = new_sal[2:len(new_sal)-4] # remove first 2 and last 4 characters
            sal = float(sal)
            number_hold = float(number_hold) # cast the both numbers into a float and times them together to get the resulting salary
            sal = sal*number_hold
            sal = str(sal)
            vars_ = vars_.upper()
            sal = sal + " " + vars_ # finally putting the currency units at the end of the string eg. 328729 INR
        except:
            statics = {"GBP":0.77, "INR":73.48,"ISK":127.27, "CAD":1.34, "EUR":0.89, "SDG":55.25, "AUD":1.52, "MXN":19.75, "SEK":9.46, "NOK":9.28, "JPY":106.83, "BRL":4.63} # if the url provides an error, use static values instead
            salary = salary[1:]
            sal = int(salary) * statics[vars2] # getting the dictionary number from the selection variable and multiplying to get a new currency salary
        ...
        tk.Label(currency_tab2, text = vars_).grid(row=1, column=1) # giving the final result back to the user
        HoverButton(currency_tab2, activebackground='blue', fg="white", bg="black", text="Back", command = lambda: [both.closetab(currency_tab2), job.display(job_name, 0, ID_insert)]).grid(row=2) # returning to the job specific homepage
        
    def listingsearch(self, result_tab, job_name, ID_insert):
        result_tab.destroy()
        f = open("adaptive-listings.txt") # opening the file containing all websites that show job listings
        name = []
        url = []
        rep = []
        for i in range(0, 1):
            temp = f.readline()
            temp = str(temp)
            temp = temp[:len(temp)-1] # get all of the indeed information including name, url, symbol to replace
            name.append(temp)
            temp = f.readline() # if this was to be expanded, the other sites can just be added to the file and number of iterations increased
            temp = str(temp)
    ...
    def copytoclip(self, url):
        ...
        r.withdraw()
        r.clipboard_clear()
        r.clipboard_append(url) # using built in tkinter methods to clear and then copy a url to a clipboard for the user to use wherever they want
        r.update()
        r.destroy()
    ...
    def listings_(self, job_name, name, url, rep, ID_insert):
        ...
        for c in range(0, len(name)):
            if VAR == name[c]:
                pos.append(c) # only using position 0 here because I have only used indeed as it was realised that alternative sites were completely unnessasary
        url = url[pos[0]]
        name = name[pos[0]]
        rep = rep[pos[0]]
        hold = job_name
        ...
        job_name = job_name.replace("/", " ") # replacing various characters in the jobname and then inserting the jobname into the url to complete the structure for indeeds system
        job_name = job_name.replace("-", " ")
        job_name = job_name.replace(",", " ")
    ...
    def listingmenu(self, url, hold, VAR, ID_insert):
        ...
        if no_result_test != []: # if this is found, the search has yielded no results and hence the user is shown "No Results", in a tkinter tab below
            ...
            try:
                ...
                locationsofjobs = soup.findAll("div", {"class": "recJobLoc"}) # any search with results will have a title and a recJobLoc
                href = soup.findAll("div", {"class": "title"}) # used to record all of the jobs in the results of the search in the url (stored twice to hold for later on) 
            except:
                carryerror = True # placeholder
            hrefs = []
            for div in href:
                link = div.find("a")["href"] # retreiving the link for the button that chooses that result
                link = "https://www.indeed.co.uk"+link # gathering full link
                hrefs.append(link)
            locationsofjobs = str(locationsofjobs)
            locationsofjobs = locationsofjobs.split("=")
            locations = []
            for u in range(0, len(locationsofjobs)): # splitting all of the locations and removing first character if a "
                if locationsofjobs[u][0] == '"':
                    locations.append(locationsofjobs[u])
            del locations [::2] 
            newlocations = locations # this removes every other item in the locations array
            all_location = []
            try:
                for k in range(0, len(locations)-1):
                    if locations[k][len(locations[k])-2:] == "id":
                        locations[k] = locations[k][1:len(locations[k])-4] # cleaning some of the locations by trimming first and last 4 characters
                        all_location.append(locations[k])
            except:
                cont = True # placeholder
            namesofjobs = str(namesofjobs)
            namesofjobs = namesofjobs.split('title="') #splitting all the job names to put into an array after filtering out specific characters
            jobnames = []
            ...
            jobnames.remove(jobnames[0]) # first item doesn't contain any job names
            ...
            while len(jobnames) > 18:
                jobnames.remove(jobnames[len(jobnames)-1]) # maximum of 18 can fit onto the size tkinter tab I have chosen
            for x in range(0, len(jobnames)):
                jobnames[x] = jobnames[x] + " --- " + all_location[x]
                allnames.append(jobnames[x]) # creating a button for each job for the user to choose and press
    ...
    def joblisting_indeed(self, hold, hrefs, jobnames, jobname, ID_insert):
        listname = jobname
        for g in range(0, len(jobnames)): # running through the names to find the position of the link in the hrefs array
    ...
    def openurl(self, href):
        try:
            import webbrowser # module that easily allows program to open a url in the users default browser
        except:
            cont = True # placeholder
        webbrowser.open(href) # openning the url
    ...
    def displayskills(self, skills, var, job_name, result_tab, ID_insert):
        ...
        tabcontrol.select(skill_tab)
        r = 0
        for n in range(0, len(skills)-1): # for each skill, split the string into parts as it must be put on seperate lines when displayed
            spaces = []
            iterate = 0
            for char in skills[n]:
                if char == " ": # split based on spaces and doesnt cut off words in the middle as a result
                    spaces.append(iterate)
                iterate+=1
            if len(spaces) >= 10 and len(spaces) <= 19:
                tk.Label(skill_tab, font="Verdana 7", text=skills[n][:spaces[9]]).grid(row=r+3, column=7) # split into 2 lines
                ...
                tk.Label(skill_tab, font="Verdana 7", text=skills[n][spaces[9]+1:spaces[18]]).grid(row=r+4, column=7) # split into 3 lines
                tk.Label(skill_tab, font="Verdana 7", text=skills[n][spaces[18]+1:]).grid(row=r+5, column=7)
            else:
                tk.Label(skill_tab, font="Verdana 7", text=skills[n]).grid(row=r+3, column=7) # small enough to keep in one line
            r = r+6
            tk.Label(skill_tab, font="Ariel 5", text=" ").grid(row=r) # adds a space inbetween each skill
    ...
    def search(self, searchjobentry, tab1, ID_insert):
        ...
        searchjobentry = searchjobentry.split(" ") # splitting each word in the input
        results = []
        for c in range(0, len(searchjobentry)):
            ...
            handle.commit()
            handle.close() #search in database word by word of each job and use LIKE and % wildcards to widen critera. If no results, option to add a job to be reviewed.
            results.append(jobnames)
        if results == [[]]:
            job.missingjob1(tab1, ID_insert) # no job found
        else:
            tab1.destroy()
            job.searchmenu(results, ID_insert) # continue to process in next function
    ...
    def searchmenu(self, results, ID_insert):
        ...
        string = str(results)
        for i in string:
            if i not in punctuation: # initally removing all unwanted characters
                new += i
        new = new.split(",,")
        for t in range(0, len(new)):
            new[t] = new[t][:len(new[t])-1] # split and removed last character
        new[len(new)-1] = new[len(new)-1][:len(new[t])-1]
        while len(new) > 1:
            new.remove(new[random.randint(0, len(new)-1)]) # removing some excess names until one is found as the result
        job_name = new[0]
        a = True
        try:
            if job_name[0] == " ":
                job_name = job_name[1:]
            if job_name[len(job_name)-1] == "\\": # cleaning spaces at the start or end of the job name
                job_name = job_name[:len(job_name)-1]
        except:
            tab1 = ttk.Frame(tabcontrol)
            a = False
            job.missingjob1(tab1, ID_insert) # if there are any errors, suggest there are no jobs
        if a == True:
            job.display(job_name, 0, ID_insert) # finally suggest the remaining job name
        
    def missingjob1(self, tab1, ID_insert):
        tab1.destroy()
        #give options 
        missing = ttk.Frame(tabcontrol)
        tabcontrol.add(missing, text="ADD JOB") # creating a tab for the user to suggest any new jobs, if they feel that the program missed an obvious role
        ...
        tk.Label(missing, font="Verdana 15", fg="Black", text=" ").grid(row=7, column=4) # spacing purposes
    ...
    def missingjob2(self, JOB__name, missing, JOB__des,  JOB_field, ID_insert):
        ...
        JOB__name = JOB__name.get() # interpret inputs
        JOB__des = JOB__des.get()
        missing.destroy()
        handle = sql.connect("RecommendDATA.db") # insert inputs into the database
        cursor = handle.cursor()
        cursor.execute("INSERT INTO USER_RECOMMENDATIONS VALUES(?,?,?)", (JOB__name, JOB__des, JOB_field))
        handle.commit()
        handle.close()
        job.mainmenu(ID_insert) # return to the job section homepage
    ...
    def popular(self, tab1, ID_insert):
        ...
        handle = sql.connect("RecommendDATA.db")
        cursor = handle.cursor()
        cursor.execute("SELECT COUNT FROM JOBS") # getting the recommendation count for each job
        counts = cursor.fetchall()
        handle.commit()
        handle.close()
        counts_array = []
        for t in range(0, len(counts)):
            ...
            counts[t] = re.sub('[^0-9]', '', counts[t]) # creating an array of all the counts after cleaning each item into only integers
        ...
        names = []
        for t in range(0, 8):
            cursor.execute("SELECT JOBNAME FROM JOBS WHERE COUNT = ? AND IDLE = ?", (counts[len(counts)-t-1], 0)) # find the job name with the highest, second-highest etc. upon iteration
            job_name = cursor.fetchall()
            handle.commit()
            if len(job_name) > 1:
                try:
                    job_name = job_name[t] # if more than one job has the same count of recommendations, choose randomly or by the number of iteration so far
                except:
            ...
            if job_name == "":
                ...
                if job_name[0] == "'": # process of cleaning the jobname from problematic characters and then adding to the final array
                    job_name = job_name[1:]
        ...
        tk.Label(new_tab, text= " ").grid(row=3) # spacing purposes
        tk.Label(new_tab, text= " ").grid(row=5)
        tk.Label(new_tab, text= "Popular Roles", font="Verdana 30 italic").grid(row=2, column=3)
        VAR = StringVar()
        VAR.set(names[random.randint(0, len(names)-1)]) # randomly display a job name from the final array as a default as there is no proirity of jobs
        tk.OptionMenu(new_tab, VAR, *names).grid(row=4, columnspan=7)
    ...
    def popular2(self, VAR, ID_insert):
        job_name = VAR.get() # interpret input
        job.display(job_name, 0, ID_insert) # run to display the job specific page, without adding to recommended total
    ...
    def fieldstart(self, tab1, ID_insert):
        ...
        for x in range(0, len(txts)):
            file = open(txts[x])
            fieldjob = file.readlines() # gather all of the field names from the first line of each of the many files (this could also be pulled from the database and eliminating duplicates)
        ...
        tk.OptionMenu(fieldsearch, variance, *fields).grid(row=4, column=3) # option menu showing all of the possible fields
        tk.Label(fieldsearch, text="   ").grid(row=1, column=1)
        tk.Label(fieldsearch, text="   ").grid(row=3, column=2) # spacing purposes between different labels and buttons
        tk.Label(fieldsearch, text="   ").grid(row=5, column=3)
        HoverButton(fieldsearch, activebackground='Light Blue', fg="white", bg="Black", text="Confirm", command = lambda: job.field_data(variance, fieldsearch, ID_insert)).grid(row=6, column=3) # continue to select jobs from chosen field
    ...
    def field_data(self, variance, fieldsearch, ID_insert):
        ...
        handle = sql.connect("RecommendDATA.db")
        cursor = handle.cursor()
        cursor.execute(("SELECT JOBNAME FROM JOBS WHERE FIELD = ? AND IDLE = ?"), (variance, 0)) # collecting all of the jobs that are associated to the selected field form the database
        names = cursor.fetchall()
        handle.commit()
        handle.close()
        punctuation = """!"#$%&'()*+:;=?@[\\]^_`{|'}~"""
        ...
        string = str(names)
        for i in string:
            if i not in punctuation: # extracting all of the unwanted characters from the string
                new += i
        new = new.split(",,") # split back to a list
        for t in range(0, len(new)):
            new[t] = new[t][:len(new[t])-1]
        new[len(new)-1] = new[len(new)-1][:len(new[t])-1]
        # need to show a menu with all jobs that is clickable into an individual menu
        var_job = StringVar()
        var_job.set(new[random.randint(0, len(new)-1)]) # setting the variable a random job from the field
        ...
        tk.Label(results_field, text="   ").grid(row=3, column=1) # blank labels provide spacing and structure to the UI
        tk.Label(results_field, text="   ").grid(row=5, column=1)
        tk.OptionMenu(results_field, var_job, *new).grid(row=4, column=3) # creating the option menu for the user to select from
    ...
    def delete_fieldresults(self, tab1, results_field):
        tab1.destroy()
        results_field.destroy() # destroying both tabs (could call the both.closetab(tab) for each individually)
        
    def getresultoffield_search(self, var_job, results_field, ID_insert):
        var_job = var_job.get()
        results_field.destroy() # interpret data and destroy the tab
        job_name = var_job
        if job_name[0] == " ": # eliminating any first character space and the running to display the job specifc page without and addition of recommendation counts
            job_name = job_name[1:]
        job.display(job_name, 0, ID_insert)

    def rating(self, tab1, ID_insert):
        tab1.destroy() # after recommendation, user gives feedback rating 0-5 and this will be used to compare jobs, that statistic is used in this searching mechanism (average ratings)
        handle = sql.connect("RecommendDATA.db")
        cursor = handle.cursor()
        cursor.execute("SELECT JOBID FROM RATINGS")
        ids = cursor.fetchall()
        handle.commit()
        handle.close() # selecting all of the job ids that have received ratings
        ident = []
        cont = True
        for h in range(0, len(ids)):
            cont = True
            ids[h] = re.sub('[^0-9]', '', str(ids[h])) # only permitting integers to be part of the string
            for v in range(0, len(ident)):
                if ident[v] == ids[h]:
                    cont = False # not allowing duplicates into the array
            if cont == True:
                ident.append(ids[h])
        averages = []
        for l in range(0, len(ident)):
            handle = sql.connect("RecommendDATA.db")
            cursor = handle.cursor()
            cursor.execute("SELECT RATING FROM RATINGS WHERE JOBID = ? AND JOBID > ?", (ident[l], 0)) # collecting all of the ratings for each job in the array of job ids
            ratings = cursor.fetchall()
            handle.commit()
            handle.close()
            total = 0
            for g in range(0, len(ratings)):
                ratings[g] = re.sub('[^0-9]', '', str(ratings[g])) # only allowing integers to be part of strings, accumilating the total ratings for each job
                total = total + int(ratings[g])
            average = total / len(ratings) # calculating the average rating by dividing the total rating by the length of string
            averages.append(average)
        temp = averages
        averages.sort()
        sort_averages = averages # using a hold, to sort one array and find the highest average in the array and therfore the position of the job id
        averages = temp
        top5 = []
        pos = []
        if len(sort_averages) > 5:
            number = 5
        elif len(sort_averages) < 5 and len(sort_averages) > 1: # variable number that can have a maximum of 5 and a minimum of 1; this is how many jobs are ultimately suggested in the option menu
            number = len(sort_averages)
        else:
            number = 1
        for y in range(0, number):
            top5.append(sort_averages[len(sort_averages)-y-1]) # collecting the top group of averages into a new array called "top 5"
        for a in range(0, len(averages)):
            for b in range(0, len(top5)):
                if averages[a] == top5[b]: # finding all of the positions that are the higest averages 
                    pos.append(a)
        final_ids = []
        for o in range(0, len(pos)):
            position = pos[o]
            id_ = ident[position] # from these postitions, extracting the id that corrsponds in the job id array
            final_ids.append(id_)
        final_ids2 = []
        for k in range(0, len(final_ids)):
            if final_ids[k] not in final_ids2: # removing all of the duplicates in the array
        ...
        for q in range(0, len(final_ids2)):
            ...
            job_name = cursor.fetchall()
            handle.commit() # for each id in the array, select the matching jobname and then format the tuple recieved back into a string
            handle.close()
            job_name = str(job_name)
            job_name = job_name[3:len(job_name)-6]
            job_names.append(job_name) # adding all finished strings back into an array for the option menu
        results_field = ttk.Frame(tabcontrol)
        tabcontrol.add(results_field, text="JOB RESULTS")
        tabcontrol.select(results_field)
        var = StringVar()
        var.set(job_names[random.randint(0, len(job_names)-1)]) # random default selection of a job name
...
class schools: # creating a school class for all appropriate functions to be catagorised under
    def home(self, IDinsert):
        self.__tab = ttk.Frame(tabcontrol)
        tabcontrol.add(self.__tab, text="Schools") # new tkinter tab for the homepage of the school section
        tabcontrol.select(self.__tab)
        tk.Label(self.__tab, text="  ").grid(row=2, column=1)
        tk.Label(self.__tab, text="  ").grid(row=4, column=3) # blank labels for structural purposes
        ...
        Postcodeentry = tk.Entry(self.__tab, font="Ariel 12") # option for the user to enter a postcode and find the closest school to it
        ...
        area_entry.insert(END, "Area Name") # the user can input an area that is confirmed and then all the schools in that area are shown in an option menu
        ...
        searchingentry.insert(END, "Name") # simple searching by name of the school to get most appropriate output
    ...
    def area(self, area_entry, tab, IDinsert):
        ...
        handle = sql.connect("Schools.db")
        cursor = handle.cursor()
        cursor.execute("SELECT LOCATION FROM DATA") # getting all of the areas that correspond to all schools
        ...
        for ar in areas:
            ar = str(ar)
            ar = ar[2:len(ar)-3] # removing all duplicaate areas from the tuple and forming a new array
            if ar not in all_areas:
                all_areas.append(ar)
        area_entry = area_entry.upper()
        for l in range(0, len(all_areas)):
            all_areas[l] = all_areas[l].upper() # making it case insensitive with it all being upper case
            repeats = 0
            for p in range(0, len(all_areas[l])):
                try:
                    letter = all_areas[l][p]
                    if area_entry[p] == letter: # finding the amount of reccuring characters between an input and an area
                        repeats = repeats+1
                except:
                    cont=True # placeholder
            repeatlist.append(repeats)
        repeatlist.sort()
        highestrepeat = repeatlist[len(repeatlist)-1] # finding the amount of repetitions of an area, with the most matches to the input
        school =[]
        for l in range(0, len(all_areas)):
            repeats = 0
            for p in range(0, len(all_areas[l])):
                try:
                    ...
                    if area_entry[p] == letter: 
                        repeats = repeats+1 # repeating the process to see which areas have the same amount of reccuring characters as the highest count
                except:
                    cont=True # placeholder
            if repeats == highestrepeat:
        ...
        tabcontrol.select(new_tab)
        tk.Label(new_tab, text= " ").grid(row=1) # blank labels for spacing and structure
        tk.Label(new_tab, text= " ").grid(row=3)
        tk.Label(new_tab, text= " ").grid(row=5)
        tk.Label(new_tab, text= "Search Result", font="Verdana 30 italic").grid(row=2, column=3)
        VAR = StringVar()
        VAR.set(school[random.randint(0, len(school)-1)]) # randomly choosing a default school as they all have the equal repetitions
    ...
    def areas2(self, area, tab, IDinsert):
        area = area.get()
        both.closetab(tab) # interpret the tkinter inputs from function "area"
        handle = sql.connect("Schools.db")
        cursor = handle.cursor()
        ...
        handle.close()
        all_names2 = []
        for name in all_names: # selecting all schools that are catagorised into that area, appending them all into a final array
            name = str(name)
        ...
        tabcontrol.select(new_tab)
        tk.Label(new_tab, text= " ").grid(row=1)
        tk.Label(new_tab, text= " ").grid(row=3) # blank labels for structural purposes
        tk.Label(new_tab, text= " ").grid(row=5)
        tk.Label(new_tab, text= "Schools", font="Verdana 30 italic").grid(row=2, column=3)
        VAR = StringVar()
        VAR.set(all_names[random.randint(0, len(all_names)-1)]) # randomly showing schools as there is no appropriate way to give proirity here
        tk.OptionMenu(new_tab, VAR, *all_names).grid(row=4, columnspan=7) # showing the user the option menu with all of the schools in the selected area
    ...
    def search(self, search, tab, IDinsert):
        ...
        handle = sql.connect("Schools.db")
        cursor = handle.cursor()
        cursor.execute("SELECT NAME FROM DATA") # getting every single school name in the entire database
        names = cursor.fetchall()
        handle.commit()
        handle.close()
        for name in names:
            name = str(name)
            name = name[2:len(name)-3]
            all_names.append(name) # collecting all school names from the tuple and forming a new array
        for l in range(0, len(all_names)):
            ...
            for p in range(0, len(all_names[l])):
                try:
                    letter = all_names[l][p]
                    if entrybox[p] == letter: # finding the amount of reccuring characters between an input and a name
                        repeats = repeats+1
                except:
                    cont=True # placeholder
            repeatlist.append(repeats)
        repeatlist.sort()
        highestrepeat = repeatlist[len(repeatlist)-1] # finding the amount of matches that is the highest of all names
        school =[]
        for l in range(0, len(all_names)):
            repeats = 0
            for p in range(0, len(all_names[l])):
                try:
                    ...
                    cont=True # placeholder
            if repeats == highestrepeat: # repeating the process to see which names have the same amount of reccuring characters as the highest count
                school.append(all_names[l])
        if len(school) > 3:
            while len(school) > 3:
                school.remove(school[len(school)-1]) # maximum of 3 schools being given
        for r in range(0, len(school)):
            schoolname = school[r]
            schools.display(self, schoolname, IDinsert)
        
    def area3(self, tab, schoolname, IDinsert): # function that takes a tkinter input, interprets it, then closes the tab that the input is from; lastly giving the display function the interpreted parameter
    ...
    def display(self, schoolname, IDinsert):
        ...
        handle.commit() # using sql to extract all the relevant data about the school from the database
        handle.close()
        details = str(details[0])
        details = details[1:len(details)-1]
        details = details.split(",") # splitting to seperate attributes of the school
        for u in range(0, len(details)):
            if details[u][0] == " " and details[u][1] == "'": # removing some unwanted characters
        ...
        handle = sql.connect("Schools.db")
        cursor = handle.cursor()
        cursor.execute("INSERT INTO RECOMMEND3 VALUES(?,?)", (details[0], IDinsert)) # record that the user has been recommended the closest school by using both ids
        handle.commit()
        handle.close()
        tab1 = ttk.Frame(tabcontrol)
        ...
        tk.Label(tab1, text=" ").grid(row=13, column=2) # labels that are blank to provide structure
        ...
        HoverButton(tab1, width=70, height=2, activebackground='Grey', fg="white", bg="Black", text="Home", command = lambda: [schools.home(self, IDinsert), both.closetab(tab1)]).grid(row=20, columnspan=7) # standard two buttons that allow conveniant navigation
    ...
    def run(self, post, tab, IDinsert):
        ...
        both.closetab(tab)
        tab1 = ttk.Frame(tabcontrol)
        tabcontrol.add(tab1, text="Schools") # new tkinter tab for the school output to be expressed on
        tabcontrol.select(tab1)
        try:
            nomi = pgeocode.Nominatim('GB')
            info = nomi.query_postal_code(post) # translating the postcode input into various pieces of data in an array, containing crucially the longitude and latitudes
            lat = info[9]
            longit = info[10]
        except:
            lat = 0
        ...
        handle = sql.connect("Schools.db")
        cursor = handle.cursor()
        cursor.execute("SELECT LATIT, LONG FROM DATA") # getting every different pairs of coordinates from each school in the database table DATA
        latlong = cursor.fetchall()
        handle.commit()
        handle.close()
        hold = 99999999999999999999 # ensuring that hold will always be higher than any real distance
        data = []
        for y in range(0, len(latlong)):
            dist = job.jobcentre.distance(self, y2, x2, latlong[y][1], latlong[y][0]) # calculate the distance between postcode and school
            if float(dist) <= float(hold):
                hold = dist
                data.append(latlong[y][0]) # recording shortest distances
                data.append(latlong[y][1])
        try:
            handle = sql.connect("Schools.db")
            cursor = handle.cursor()
            cursor.execute("SELECT SCHOOLID, NAME, LOCATION, TYPE, GENDER, LONG, LATIT, EAST, NORTH, LOWAGE, HIGHAGE, OPEN, WEBSITE FROM DATA WHERE LONG = ? AND LATIT = ?", (data[len(data)-1], data[len(data)-2])) # get all of the data required for the schools page
            details = cursor.fetchall()
            handle.commit()
            handle.close()
            details = str(details[0])
            details = details[1:len(details)-1] # splitting the seperate attributes of a school
            details = details.split(",")
            for u in range(0, len(details)):
                if details[u][0] == " " and details[u][1] == "'":
                    details[u] = details[u][2:len(details[u])-1] # removing some unwanted characters
                elif details[u][0] == " ":
                    details[u] = details[u][1:]
                elif details[u][0] == "'":
                    details[u] = details[u][1:len(details[u])-1]
